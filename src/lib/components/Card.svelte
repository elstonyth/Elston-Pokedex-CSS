<script>
  ...
  // Derived values (avoid mutating props)
  let normalizedRarity;
  let normalizedSupertype;
  ...
  // Throttle tracking - ensures 60fps max
  let lastFrameTime = 0;
  ...
  const interact = (e) => {
    // Double throttle: RAF + frame time check for guaranteed 60fps
    const now = performance.now();
    if (rafId || (now - lastFrameTime) < FRAME_TIME) return;
    ...
    rafId = requestAnimationFrame(() => {
      ...
      // Cache bounding rect for performance - avoid layout thrashing
      if (!cachedRect) {
  ...
  const interactEnd = (e, delay = 500) => {
    // Clear cached rect and RAF on interaction end
    cachedRect = null;
    if (rafId) {
    ...
    setTimeout(function () {
      // Faster snap-back with higher precision threshold for better FPS
      const snapStiff = 0.02;
      const snapDamp = 0.12;
      interacting = false;
  ...
  const retreat = () => {
    ...
    // Reset firstPop so the card can flip again on next activation
    firstPop = true;
  };
  ...
  $: {
    normalizedRarity = rarity?.toLowerCase() || 'common';
    normalizedSupertype = supertype?.toLowerCase() || 'pokémon';
    normalizedNumber = number?.toString().toLowerCase() || '';
    normalizedTypes = Array.isArray(types) ? types.join(" ").toLowerCase() : (types?.toLowerCase() || '');
    isTrainerGallery = !!normalizedNumber.match(/^[tg]g/i) || id === "swshp-SWSH076" || id === "swshp-SWSH077";
  }
...
<div
  ...
  data-set={set}
  data-subtypes={Array.isArray(subtypes) ? subtypes.join(' ').toLowerCase() : subtypes}
